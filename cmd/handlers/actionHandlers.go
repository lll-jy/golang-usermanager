...
func DecryptPhoto(url string, pass string, name string, photo *string) error {
	if url == "" || url == paths.PlaceholderPath {
		*photo = paths.PlaceholderPath
		...
		if err != nil {
			go log.Printf("The encrypted file %s is invalid because %s.", url, err.Error())
			return err
		}
		...
		*photo = fmt.Sprintf("%s/user%s.jpeg", paths.TempPath, name)
		...
		if err != nil {
			go log.Printf("Cannot write file. %s.", err.Error())
			return err
		}
	}
	return nil
...
func LoginHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if protocol.IsExistingUsername(db, name, &u) {
		go log.Printf("User %s found.", name)
		if protocol.IsCorrectPassword(pass, u.Password) {
			go log.Printf("Login to %s successful!", name)
			u.Name = name
			...
			if err != nil {
				go log.Printf("Error: wrong format! %v cannot be parsed as a user.", &u)
			}
			header.Set("user", string(user))
			header.Set("status", "successful login")
		} else {
			go log.Printf("Login to %s unsuccessful due to wrong password!", name)
			tu.Password = ""
			ie.PasswordErr = "Incorrect password."
			header.Set("status", "incorrect password")
		}
	} else {
		go log.Printf("User %s does not exists. Redirect to sign up page.", name)
...
func LogoutHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	info := GetPageInfo(r)
	clearSession(w)
	go log.Printf("User %s logged out.", info.User.Name)
...
func userInfoHandler(db *sql.DB, w http.ResponseWriter, r *http.Request, rt string, tgt string, query string) {
	...
	if protocol.IsValidUsername(name) {
		if protocol.IsExistingUsername(db, name, u) {
			go log.Printf("User signup failure: duplicate user %s found.", name)
			...
			if pass == repeatPass {
				if rt == "/signup" {
					go log.Printf("New user %s signed up.", name)
				}
				...
				if info.Photo != "" && info.Photo != paths.PlaceholderPath {
					...
					if err != nil {
						go log.Printf("The original file path is invalid.")
					}
					fileBytes, err := ioutil.ReadFile(info.Photo)
					if err != nil {
						go log.Printf("The temporary file cannot be read properly.")
					}
					err = ioutil.WriteFile(u.PhotoUrl, encrypt(fileBytes, pass), 0600)
					if err != nil {
						go log.Printf("The file cannot be re-encrypted.")
					}
					go log.Printf("The original file key is updated.")
				}
				...
				go log.Printf("User signup/reset failure: password does not match.")
				ie.PasswordRepeatErr = "The password does not match."
				header.Set("status", "mismatch password")
			}
		} else {
			go log.Printf("User signup/reset failure: password format invalid.")
			u.Name = name
			ie.PasswordErr = "The password is not valid."
			header.Set("status", "wrong password format")
		}
	} else {
		go log.Printf("User signup/reset failture: invalid username format of %s.", name)
...
func EditHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if info.User.Password != "" {
		...
		if len(info.TempUser.Nickname) > 20 {
			...
			go log.Printf("User information of %s updated.", info.User.Name)
			if info.User.PhotoUrl != "" && info.User.PhotoUrl != paths.PlaceholderPath {
				err := os.Remove(info.User.PhotoUrl)
				if err == nil {
					go log.Printf("Removed original photo from database.")
				} else {
					go log.Printf(err.Error())
				}
			}
...
func UploadHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if info.User.Password != "" {
		...
		if err != nil {
			go log.Println("Error retrieving file.")
			header.Set("status", fmt.Sprintf("cannot retrieve: %s ||\n", err))
		} else {
			defer file.Close()
			go log.Printf("Photo %s uploaded for user %s. The file size is %+v. MIME header is %+v.", handler.Filename, info.User.Name, handler.Size, handler.Header)
			targetDir := paths.FileBasePath
			tempFile, err := ioutil.TempFile(targetDir, "upload-*.jpeg")
			if err != nil {
				go log.Printf("Error generating temporary file. %s.", err.Error())
			}
			defer tempFile.Close()
			fileBytes, err := ioutil.ReadAll(file)
			if err != nil {
				go log.Println("Error reading file.")
			}
			...
			setSession(info.User, info.TempUser, info.InfoErr, info.Photo, w)
			go log.Println("Successfully uploaded file")
			header.Set("tempPhoto", info.Photo)
...
func DiscardHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if info.User.Password != "" {
		if info.User.PhotoUrl != "" && info.User.PhotoUrl != paths.PlaceholderPath {
			...
			go log.Printf("Temporary file removed.")
		} else {
			go log.Printf("No file to remove.")
		}
...
func RemoveHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if info.User.Password != "" {
		...
		http.Redirect(w, r, "/edit", 302)
		go log.Printf("Removed profile photo for user %s", info.User.Name)
	} else {
		http.Redirect(w, r, "/", 302)
	}
}
...
func DeleteHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	if info.User.Password != "" {
		...
		if err != nil {
			go log.Printf("Query %s cannot be executed due to error: %s", query, err.Error())
			header.Set("status", "cannot delete")
		} else {
			go log.Printf("User %s deleted.", name)
